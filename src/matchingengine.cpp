...
void MatchingEngine::processOrder(Order order){
    if(order.side == Side::BUY){
        ...
        while(it != orderBook.sells.end() && it -> price <= order.price && order.quantity > 0){
            ...
            trades.emplace_back(nextTradeId++, order.orderId, it -> orderId, tradeQuantity, tradePrice, order.timestamp);
        ...
        while(it != orderBook.buys.end() && it -> price >= order.price && order.quantity > 0){
            ...
            double tradePrice = it -> price;
            trades.emplace_back(nextTradeId++, it -> orderId, order.orderId, tradeQuantity, tradePrice, order.timestamp);
            order.quantity -= tradeQuantity;
